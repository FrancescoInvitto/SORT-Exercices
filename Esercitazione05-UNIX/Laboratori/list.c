...
{
  if ( item1.num_pezzi > item2.num_pezzi) 
    return 1;
  else if ( item1.num_pezzi < item2.num_pezzi ) 
    return -1;
  else
    return 0;
}
...
{
  printf("PID: %d, Tipo lavorazione: %d, Tipo processo: %d, Numero pezzi: %d, Sockfd: %d\n", item.pid, item.lavorazione, item.tipo, item.num_pezzi, item.sockfd);
}
